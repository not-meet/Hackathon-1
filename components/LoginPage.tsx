...
export default function LoginPage() {
  ...
  const handleLogin = async (event: React.FormEvent) => {
    ...
    try {
      ...
      const response = await axios.post('https://localhost/api/auth/mentorSignin', {
        email,
        password,
        role: activeTab
      })
          ...
          <CardContent>
            <Tabs value={activeTab} onValueChange={handleTabChange as (value: string) => void} className="w-full">
              ...
              <TabsContent value="mentee">
                ...
                <p className="mt-6 text-center text-sm text-blue-600">
                  Don't have an account?{" "}
                  <Link href="/app/signup" className="font-medium text-blue-600 hover:underline transition-colors duration-300">
                    Sign up
                  </Link>
                </p>
              </TabsContent>
